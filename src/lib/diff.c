 ...
 */
typedef struct {
	edj_t	*key;	/* For objects, this is the key; for arrays, ignored */
...
{
	return strcmp((*(edj_t **)p1)->text, (*(edj_t **)p2)->text);
}

/* Given an object, return a sorted array of its member EDJ_KEY nodes.  The
 * end of the array will be indicated by a NULL pointer.
 */
static edj_t **sort_keys(edj_t *obj)
{
	edj_t	*mem;
	int	i;
	int	len = edj_length(obj);
	edj_t	**sorted = malloc((len + 1) * sizeof(edj_t*));
	for (mem = obj->first, i = 0; mem; mem = mem->next, i++)
		sorted[i] = mem;
	sorted[i] = NULL;
	qsort(sorted, len, sizeof(edj_t *), cmp_keys);
	return sorted;
}

/* This basically computes a fletcher-style checksum of the words in an edj_t.
 * If it is an array, object, or key then the contents are also added to the
 * sum.  You should pass 0 for the seed.
 */
int edj_hash(edj_t *json, int seed)
{
	int	hash = seed;
	...
	if (json->type == EDJ_STRING || json->type == EDJ_KEY || (json->type == EDJ_OBJECT && *json->text)) {
	...
	else if (json->type == EDJ_BOOLEAN) {
		/* For booleans, add the "t" or "f" */
		sum += *json->text;
		sumsum += sum;
	} else if (json->type == EDJ_NUMBER) {
		...
		if (*json->text) {
			...
			if (json->text[pos]) {
				EDJ_DOUBLE(&tmpbuf) = atof(json->text);
				tmpbuf.text[1] = 'd';
			} else {
				EDJ_INT(&tmpbuf) = atoi(json->text);
				tmpbuf.text[1] = 'i';
			}
	...
	if (json->first) {
		 ...
		 */
		if (json->type == EDJ_OBJECT) {
			/* Build a sorted list of members */
			int i;
			edj_t **sorted = sort_keys(json);

			/* Incorporate the sums for each key (and its value) */
			for (i = 0; sorted[i]; i++)
				hash = edj_hash(sorted[i], hash);
...
static edjcolumn_t j_to_c(edj_t *json)
{
	edj_t	*scan;
	edjcolumn_t col;
	int	i;
	...
	if (json->type == EDJ_OBJECT) {
		for (i = 0, scan = json->first; scan; i++, scan = scan->next) {
			col.array[i].hash = edj_hash(scan, 0);
			col.array[i].key = scan;
		}
		qsort(col.array, col.len, sizeof *col.array, cmp_keys);
	} else {
		for (i = 0, scan = edj_first(json); scan; i++, scan = edj_next(scan)) {
			col.array[i].hash = edj_hash(scan, 0);
		}
	}
...
/* Return the index as an edj_t.  When comparing objects, this will be a string
 * containing the member's key.  For arrays, this will be a number.
 */
static edj_t *diff_index(edjcolumn_t *col, int offset)
{
	if (col->array[col->cur + offset].key)
		return edj_string(col->array[col->cur + offset].key->text, -1);
	return edj_from_int(col->cur + offset);
}

/* Return a value trying to limit the width.  For arrays/objects, convert to
 * string
 */
static edj_t *diff_value(edjcolumn_t *col, int offset, int width)
{
	...
	if (value)
		...
		value = edj_by_index(col->json, col->cur + offset);

	/* For simple numbers/symbols, just return it.  Otherwise get text */
	if (value->type == EDJ_STRING)
		text = value->text;
	else if (value->type == EDJ_ARRAY || value->type == EDJ_OBJECT)
		text = mustfree = edj_serialize(value, NULL);
	else {
		str = edj_copy(value);
		edj_break(value);
		return str;
 ...
 */
static void add_diff(edjdiff_t *dp, int oldspan, int newspan)
{
	edj_t	*entry;
	char	*edit;
	int	i;
	...
	if (dp->diffstyle & EDJ_DIFF_SPAN) {
		 ...
		 * index numbers.
		 */
		edj_append(entry, edj_key("old", edj_from_int(dp->oldc.cur)));
		edj_append(entry, edj_key("oldspan", edj_from_int(oldspan)));
		if (dp->diffstyle & EDJ_DIFF_EDIT) {
			if (oldspan == 0)
				edit = "insert";
			else if (newspan == 0)
				edit = "delete";
				...
				edit = "change";
			edj_append(entry, edj_key("edit", edj_string(edit, -1)));
		}
		edj_append(entry, edj_key("new", edj_from_int(dp->newc.cur)));
		edj_append(entry, edj_key("newspan", edj_from_int(newspan)));

		/* Add the entry to the result, and adjust "cur" */
		edj_append(dp->result, entry);
		dp->oldc.cur += oldspan;
		dp->newc.cur += newspan;
		return;
	}
	...
	if (dp->diffstyle & EDJ_DIFF_BESIDE) {
		/* Add a "change" entry for each pair of overlapping lines */
		for (i = 0; i < oldspan && i < newspan; i++) {
			entry = edj_object();
			edj_append(entry, edj_key("old", diff_index(&dp->oldc, i)));
			if (dp->diffstyle & EDJ_DIFF_VALUE)
				edj_append(entry, edj_key("oldvalue", diff_value(&dp->oldc, i, dp->valuewidth)));
			if (dp->diffstyle & EDJ_DIFF_EDIT)
				edj_append(entry, edj_key("edit", edj_string("change", -1)));
			edj_append(entry, edj_key("new", diff_index(&dp->newc, i)));
			if (dp->diffstyle & EDJ_DIFF_VALUE)
				edj_append(entry, edj_key("newvalue", diff_value(&dp->newc, i, dp->valuewidth)));
			edj_append(dp->result, entry);
		}
		...
		for (; i < oldspan; i++) {
			...
			if (dp->diffstyle & EDJ_DIFF_VALUE)
				edj_append(entry, edj_key("oldvalue", diff_value(&dp->oldc, i, dp->valuewidth)));
			if (dp->diffstyle & EDJ_DIFF_EDIT)
				edj_append(entry, edj_key("edit", edj_string("delete", -1)));
			edj_append(entry, edj_key("new", edj_null()));
			if (dp->diffstyle & EDJ_DIFF_VALUE)
				edj_append(entry, edj_key("newvalue", edj_null()));
			edj_append(dp->result, entry);
		}
		...
		for (; i < newspan; i++) {
			entry = edj_object();
			edj_append(entry, edj_key("old", edj_null()));
			if (dp->diffstyle & EDJ_DIFF_VALUE)
				edj_append(entry, edj_key("oldvalue", edj_null()));
			if (dp->diffstyle & EDJ_DIFF_EDIT)
				edj_append(entry, edj_key("edit", edj_string("insert", -1)));
			edj_append(entry, edj_key("new", diff_index(&dp->newc, i)));
			if (dp->diffstyle & EDJ_DIFF_VALUE)
				edj_append(entry, edj_key("newvalue", diff_value(&dp->newc, i, dp->valuewidth)));
			edj_append(dp->result, entry);
	...
	for (i = 0; i < oldspan; i++) {
		entry = edj_object();
		edj_append(entry, edj_key("index", diff_index(&dp->oldc, i)));
		edj_append(entry, edj_key("edit", edj_string("delete", -1)));
		if (dp->diffstyle & EDJ_DIFF_VALUE)
			edj_append(entry, edj_key("value", diff_value(&dp->newc, i, dp->valuewidth)));
		edj_append(dp->result, entry);
	}
	for (i = 0; i < newspan; i++) {
		entry = edj_object();
		edj_append(entry, edj_key("index", diff_index(&dp->newc, i)));
		edj_append(entry, edj_key("edit", edj_string("delete", -1)));
		if (dp->diffstyle & EDJ_DIFF_VALUE)
			edj_append(entry, edj_key("value", diff_value(&dp->newc, i, dp->valuewidth)));
		edj_append(dp->result, entry);
...
edj_t *edj_diff(edj_t *oldj, edj_t *newj, edjdiffstyle_t diffstyle)
{
	edjdiff_t d;
	int	reach, check;
	edjhash_t *oldbase, *newbase;

	/* Check arguments for compatibility */
	if (oldj->type == EDJ_ARRAY && newj->type == EDJ_ARRAY) {
		/* Good! */
	} else if (oldj->type == EDJ_OBJECT && newj->type == EDJ_OBJECT) {
		/* Also good, but can't do "span" or "context" styles */
		diffstyle &= ~(EDJ_DIFF_SPAN | EDJ_DIFF_CONTEXT);
	} else {
		return edj_error_null(NULL, "diffargs:Incompatible args for diff");
	}
	...
	d.valuewidth = (diffstyle & EDJ_DIFF_BESIDE) ? 32 : 60;
	d.result = edj_array();
